...
import {
  ...
  getSceneDetails,
  deleteScene,
} from "@/services/dashboard/project";
...
export default function ProjectsClient({
      ...
      try {
        if (activeTab === "projects") {
          ...
          if (scenesRes?.success) {
            const scenesData =
              scenesRes.data?.data?.data || scenesRes.data?.data || [];
  ...
  const handleViewScene = async (sceneId) => {
    ...
    if (res?.success && res.data?.data?.length > 0) {
      ...
      // Or maybe it returns object directly
      ...
      {activeTab === "projects" ? (
        <div>
          ...
          >
            ...
            {activeTab === "projects" &&
              !searchQuery &&
              renderPagination(projectPagination, onProjectPageChange)}
          </div>
        </div>
      ) : (
        <div>
          ...
          >
            ...
            {activeTab === "scenes" &&
              !searchQuery &&
              renderPagination(scenePagination, setCurrentScenePage)}
          </div>
        </div>
      )}
