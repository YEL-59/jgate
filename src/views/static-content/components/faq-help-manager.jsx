...
export function FaqHelpManager({ type, title }) {
    ...
    const handleViewDetails = async (item) => {
        ...
        if (res?.success && res?.data) {
            setViewingItemData({ ...res.data, fallbackCreatedAt: item.created_at, fallbackUpdatedAt: item.updated_at });
        ...
        <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                ...
                <Button 
                    ...
                    style={{ backgroundColor: '#301960', color: 'white', display: 'flex', gap: '8px', alignItems: 'center' }}
                >
            ...
            {loading ? (
                ...
                <div className="custom-scrollbar" style={{ display: 'flex', flexDirection: 'column', gap: '16px', maxHeight: '500px', overflowY: 'auto', paddingRight: '4px' }}>
                    <style>{`
                    ...
                    {data.map((item) => (
                        <div key={item.id} style={{ 
                            ...
                            borderRadius: '8px', 
                            padding: '16px',
                            backgroundColor: item.status === 'inactive' ? '#f9fafb' : 'white',
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '16px' }}>
                                ...
                                <div style={{ display: 'flex', gap: '8px' }}>
                                    <button 
                                        ...
                                        title="View Details"
                                        style={{ background: 'transparent', border: 'none', cursor: 'pointer', padding: '4px', color: '#8b5cf6' }}
                                    >
                                        <Eye size={18} />
                                    </button>
                                    <button 
                                    ...
                                    <button 
                                        ...
                                        title="Edit"
                                        style={{ background: 'transparent', border: 'none', cursor: 'pointer', padding: '4px', color: '#3b82f6' }}
                                    >
                                        <Edit2 size={18} />
                                    </button>
                                    <button 
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <h3 style={{ fontSize: '20px', fontWeight: '600', margin: '0 0 20px 0' }}>
                            {editingItem ? 'Edit' : 'Add'} {title.includes('FAQ') ? 'FAQ' : 'Item'}
                        </h3>
                        
                        <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                            <div>
                                <label style={{ display: 'block', marginBottom: '8px', fontWeight: '500', fontSize: '14px' }}>Question</label>
                                <input 
                                    type="text" 
                                    value={formData.question}
                                    onChange={(e) => setFormData({...formData, question: e.target.value})}
                                    required
                                    style={{ width: '100%', padding: '10px 12px', border: '1px solid #e5e5e5', borderRadius: '6px', outline: 'none' }}
                                    placeholder="Enter question"
                                />
                            </div>
                            
                            <div>
                                <label style={{ display: 'block', marginBottom: '8px', fontWeight: '500', fontSize: '14px' }}>Answer</label>
                                <textarea 
                                    value={formData.answer}
                                    onChange={(e) => setFormData({...formData, answer: e.target.value})}
                                    required
                                    rows={4}
                                    style={{ width: '100%', padding: '10px 12px', border: '1px solid #e5e5e5', borderRadius: '6px', outline: 'none', resize: 'vertical' }}
                            ...
                            <div>
                                ...
                                <select 
                                    ...
                                    style={{ width: '100%', padding: '10px 12px', border: '1px solid #e5e5e5', borderRadius: '6px', outline: 'none', backgroundColor: 'white' }}
                                >
                            ...
                            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '16px' }}>
                                <Button type="button" onClick={handleCloseModal} style={{ backgroundColor: '#f3f4f6', color: '#4b5563', border: 'none' }}>
                                    Cancel
                                </Button>
                                <Button type="submit" disabled={saving} style={{ backgroundColor: '#301960', color: 'white', border: 'none' }}>
                                    {saving ? 'Saving...' : 'Save'}
                                </Button>
                            </div>
                        </form>
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                            ...
                            <span style={{ 
                                ...
                                fontSize: '12px',
                                fontWeight: '500',
                                backgroundColor: viewingItemData.status === 'active' ? '#d1fae5' : '#f3f4f6', 
                                color: viewingItemData.status === 'active' ? '#059669' : '#6b7280'
                            }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                            <div>
                                <h4 style={{ fontSize: '14px', color: '#6b7280', margin: '0 0 4px 0' }}>Question</h4>
                                <p style={{ fontSize: '16px', color: '#1a1a1a', margin: 0, fontWeight: '500' }}>{viewingItemData.question}</p>
                            </div>
                            
                            <div>
                                <h4 style={{ fontSize: '14px', color: '#6b7280', margin: '0 0 4px 0' }}>Answer</h4>
                                <p style={{ fontSize: '15px', color: '#4b5563', margin: 0, lineHeight: '1.6' }}>{viewingItemData.answer}</p>
                            </div>

                            <div style={{ display: 'flex', gap: '24px', marginTop: '12px', paddingTop: '16px', borderTop: '1px solid #e5e5e5' }}>
                                <div>
                                    <h4 style={{ fontSize: '12px', color: '#9ca3af', margin: '0 0 4px 0' }}>Created At</h4>
                                    <p style={{ fontSize: '13px', color: '#6b7280', margin: 0 }}>
                                        {viewingItemData.created_at || viewingItemData.fallbackCreatedAt ? new Date(viewingItemData.created_at || viewingItemData.fallbackCreatedAt).toLocaleString() : 'N/A'}
                                    </p>
                                </div>
                                <div>
                                    <h4 style={{ fontSize: '12px', color: '#9ca3af', margin: '0 0 4px 0' }}>Updated At</h4>
                                    <p style={{ fontSize: '13px', color: '#6b7280', margin: 0 }}>
                                        {viewingItemData.updated_at || viewingItemData.fallbackUpdatedAt ? new Date(viewingItemData.updated_at || viewingItemData.fallbackUpdatedAt).toLocaleString() : 'N/A'}
                                    </p>
                                </div>
                            </div>

                            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '16px' }}>
                                <Button onClick={() => setIsViewModalOpen(false)} style={{ backgroundColor: '#f3f4f6', color: '#4b5563', border: 'none' }}>
                                    Close
                                </Button>
                            </div>
                        </div>
